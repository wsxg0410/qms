...
export const queue = {
  ...
  getList: defineAction({
    ...
    handler: async (input, ctx) => {
      ...
      const result = await svc.list(input.env, {
        type: input.type,
  ...
  update: defineAction({
    ...
    handler: async (input, ctx) => {
      ...
      if (
        ...
        return svc.updateQueue(input.env, input.id, {
          status: input.status,
          result: input.result,
      ...
      if (input.status !== undefined) {
        return svc.updateStatus(input.env, input.id, input.status);
      }
  ...
  delete: defineAction({
    ...
    handler: async (input, ctx) => {
      const svc = getService(ctx.request);
      await svc.remove(input.env, input.id);
      return { id: input.id };
    },
  }),
  ...
  deleteMany: defineAction({
    ...
    handler: async (input, ctx) => {
      const svc = getService(ctx.request);
      await Promise.all(input.ids.map((id) => svc.remove(input.env, id)));
